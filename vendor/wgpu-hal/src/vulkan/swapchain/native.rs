...
impl Surface for NativeSurface {
    ...
    unsafe fn create_swapchain(
        ...
        let observer_configuration = if crate::vulkan::present_wait_observer::installed() {
            // SAFETY: this surface, physical device, entry, and instance are
            // all owned by the live WGPU objects used to create the swapchain.
            let capabilities = unsafe {
                crate::vulkan::present_wait_observer::query_surface_configuration(
                    &self.instance.entry,
                    &self.instance.raw,
            ...
            .map_err(|_| {
                crate::SurfaceError::Other(
                    "presentation timing surface-capability query failed",
                )
            })?;
            if !capabilities.present_id2_supported
                || !capabilities.present_timing_supported
                || capabilities.present_stage_queries
                    & crate::vulkan::present_wait_observer::PRESENT_STAGE_IMAGE_FIRST_PIXEL_OUT
                    == 0
            {
                return Err(crate::SurfaceError::Other(
                    "surface lacks required present-id2 or first-pixel-out timing support",
                ));
            }
            raw_flags |= crate::vulkan::present_wait_observer::observer_swapchain_flags();
            Some(capabilities)
        } else {
            None
        };
        ...
        if old_swapchain != vk::SwapchainKHR::null() {
            crate::vulkan::present_wait_observer::before_swapchain_destroy(old_swapchain);
        ...
        if let Some(mut configuration) = observer_configuration {
            // SAFETY: the extension and feature chain were required at device
            // creation and `raw` is the newly created live swapchain.
            let timing_device = match unsafe {
                crate::vulkan::present_wait_observer::PresentationTimingDevice::new(
                ...
                Ok(device) => device,
                Err(error) => {
                    unsafe { functor.destroy_swapchain(raw, None) };
                    return Err(crate::SurfaceError::Other(error));
                }
            };
            let (time_domain, time_domain_id) = match unsafe {
                timing_device.configure_swapchain(raw)
            } {
                Ok(configuration) => configuration,
                Err(_) => {
                    unsafe { functor.destroy_swapchain(raw, None) };
                    return Err(crate::SurfaceError::Other(
                        "failed to configure the swapchain presentation-timing queue or time domain",
                    ));
                }
            };
            configuration.time_domain = time_domain;
            configuration.time_domain_id = time_domain_id;
            crate::vulkan::present_wait_observer::presentation_timing_configured(
                raw,
                configuration,
            );
        }
...
impl Swapchain for NativeSwapchain {
    ...
    unsafe fn delete_swapchain(self: Box<Self>) {
        crate::vulkan::present_wait_observer::before_swapchain_destroy(self.raw);
    ...
    unsafe fn present(
        ...
        let reserved_present = crate::vulkan::present_wait_observer::reserve(self.raw);
        let present_ids;
        let mut present_id_info;
        let mut present_id2_info;
        let present_timing_infos;
        let mut present_timings_info;
        if let Some(reserved) = reserved_present.as_ref() {
            present_ids = [reserved.present_id()];
            present_id_info = vk::PresentIdKHR::default().present_ids(&present_ids);
            vk_info = vk_info.push_next(&mut present_id_info);
            present_id2_info =
                crate::vulkan::present_wait_observer::PresentId2Info::new(&present_ids);
            present_timing_infos = [
                crate::vulkan::present_wait_observer::PresentTimingInfo::first_pixel_out(
                    reserved.time_domain_id(),
                ),
            ];
            present_timings_info =
                crate::vulkan::present_wait_observer::PresentTimingsInfo::new(
                    &present_timing_infos,
                );
            present_id2_info.p_next = vk_info.p_next;
            present_timings_info.p_next = core::ptr::from_ref(&present_id2_info).cast();
            vk_info.p_next = core::ptr::from_ref(&present_timings_info).cast();
        }

        let mut display_timing;
        let present_times;
        if let Some(present_time) = self.next_present_time.take() {
            debug_assert!(
                self.device
                    .features
            ...
            vk_info = vk_info.push_next(&mut display_timing);
        }
        ...
        let suboptimal = match present_result {
            Ok(suboptimal) => {
                if let Some(reserved) = reserved_present {
                    reserved.submitted();
                }
                suboptimal
            }
            Err(error) => {
                if let Some(reserved) = reserved_present {
                    reserved.rejected(error);
                }
                return Err(match error {
                    vk::Result::ERROR_OUT_OF_DATE_KHR => crate::SurfaceError::Outdated,
                    vk::Result::ERROR_SURFACE_LOST_KHR => crate::SurfaceError::Lost,
                    error if error.as_raw() == -1_000_208_000 => crate::SurfaceError::Other(
                        "presentation timing result queue was exhausted",
                    ),
                    // We don't use VK_EXT_full_screen_exclusive
                    // VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT
