...
impl super::Adapter {
    ...
    pub unsafe fn open_with_callback<'a>(
        ...
        let presentation_wait_enabled = super::present_wait_observer::installed();
        let mut present_id_feature = vk::PhysicalDevicePresentIdFeaturesKHR::default();
        let mut present_wait_feature = vk::PhysicalDevicePresentWaitFeaturesKHR::default();
        let mut present_id2_feature =
            super::present_wait_observer::PhysicalDevicePresentId2Features::default();
        let mut present_timing_feature =
            super::present_wait_observer::PhysicalDevicePresentTimingFeatures::default();
        if presentation_wait_enabled {
            if !self
                .phd_capabilities
                .supports_extension(khr::present_id::NAME)
                || !self
                    .phd_capabilities
                    .supports_extension(khr::present_wait::NAME)
                || !self
                    .phd_capabilities
                    .supports_extension(super::present_wait_observer::present_id2_extension_name())
                || !self.phd_capabilities.supports_extension(
                    super::present_wait_observer::present_timing_extension_name(),
                )
                || !self
                    .phd_capabilities
                    .supports_extension(khr::calibrated_timestamps::NAME)
                || !self
                    .instance
                    .extensions
            ...
            {
                log::error!(
                    "the installed presentation observer requires VK_KHR_present_id, VK_KHR_present_wait, VK_KHR_present_id2, VK_KHR_calibrated_timestamps, VK_KHR_get_surface_capabilities2, and VK_EXT_present_timing"
                );
                return Err(crate::DeviceError::Lost);
            }
            let mut supported_id = vk::PhysicalDevicePresentIdFeaturesKHR::default();
            let mut supported_wait = vk::PhysicalDevicePresentWaitFeaturesKHR::default();
            let mut supported_id2 =
                super::present_wait_observer::PhysicalDevicePresentId2Features::default();
            let mut supported_timing =
                super::present_wait_observer::PhysicalDevicePresentTimingFeatures::default();
            let mut supported = vk::PhysicalDeviceFeatures2::default()
                .push_next(&mut supported_id)
                .push_next(&mut supported_wait);
            supported_timing.p_next = supported.p_next;
            supported_id2.p_next = core::ptr::from_mut(&mut supported_timing).cast();
            supported.p_next = core::ptr::from_mut(&mut supported_id2).cast();
            unsafe {
                self.instance
                    .raw
                    .get_physical_device_features2(self.raw, &mut supported)
            };
            if supported_id.present_id != vk::TRUE
                || supported_wait.present_wait != vk::TRUE
                || supported_id2.present_id2 != vk::TRUE
                || supported_timing.present_timing != vk::TRUE
            {
                log::error!(
                    "the installed presentation observer requires presentId, presentWait, presentId2, and presentTiming features"
                );
                return Err(crate::DeviceError::Lost);
            }
            enabled_extensions.push(khr::present_id::NAME);
            enabled_extensions.push(khr::present_wait::NAME);
            enabled_extensions.push(super::present_wait_observer::present_id2_extension_name());
            enabled_extensions.push(super::present_wait_observer::present_timing_extension_name());
            if !enabled_extensions.contains(&khr::calibrated_timestamps::NAME) {
                enabled_extensions.push(khr::calibrated_timestamps::NAME);
            }
            present_id_feature.present_id = vk::TRUE;
            present_wait_feature.present_wait = vk::TRUE;
            present_id2_feature.present_id2 = vk::TRUE;
            present_timing_feature.present_timing = vk::TRUE;
        }
        ...
        if presentation_wait_enabled {
            info = info
                .push_next(&mut present_id_feature)
                .push_next(&mut present_wait_feature);
            present_timing_feature.p_next = info.p_next as *mut _;
            present_id2_feature.p_next = core::ptr::from_mut(&mut present_timing_feature).cast();
            info.p_next = core::ptr::from_mut(&mut present_id2_feature).cast();
        }
        let raw_device = {
