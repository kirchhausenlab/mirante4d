...
use mirante4d_application::{
    AnalysisPlot, AnalysisPlotId, AnalysisPlotPointSelection, AnalysisTable,
    AnalysisWorkspaceSnapshot, ApplicationCommand, IntensityStatistics,
};

use crate as ui_kit;
use crate::{AnalysisPlotViewRange, AnalysisTableSort, EguiUiState, StatusTone};
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for (index, entry) in view.tables().iter().enumerate() {
                        ...
                        {
                            commands
                                .push(ApplicationCommand::SelectAnalysisTable(Some(entry.id())));
                        }
                    }
                });
        });
...
mod tests {
    ...
    fn workspace_helpers_are_typed_to_exact_core_results() {
        ...
        let _: fn(
            ...
            egui::Rect,
            egui::Pos2,
        ) -> Option<AnalysisPlotNearestPoint> = nearest_analysis_plot_point;
        let _: fn(&AnalysisTable, &str, Option<&AnalysisTableSort>) -> AnalysisTablePreviewRows =
            analysis_table_preview_rows;
    }
}
