use glam::DVec3;
use mirante4d_core::{
    CameraState, DatasetId, DisplayWindow, GridToWorld, LayerId, PresentationViewport, Projection,
...
fn orthographic_dolly_does_not_change_camera_mip_pixels() {
    ...
    let height = volume
        ...
        .length();
    let far_camera = CameraState::new(
        Projection::Orthographic,
        volume
            .grid_to_world
            .transform_point(DVec3::new(center.x, center.y, center.z - 120.0)),
        volume.grid_to_world.transform_point(center),
...
fn camera_mip_changes_when_view_direction_changes() {
    ...
    let height = volume
        ...
        .transform_vector(DVec3::Y * 16.0)
        .length();
    let side_camera = CameraState::new(
        Projection::Orthographic,
        volume
            .grid_to_world
            .transform_point(DVec3::new(center.x + 120.0, center.y, center.z)),
        volume.grid_to_world.transform_point(center),
        volume.grid_to_world.transform_vector(DVec3::Y).normalize(),
        1.0,
        height / (2.0 * (std::f64::consts::FRAC_PI_3 * 0.5).tan()),
...
fn center_aligned_downsampled_lod_does_not_shift_under_fixed_oblique_camera() {
    ...
    let s1_grid_to_world = s0_grid_to_world
        ...
        .unwrap();
    let stale_s1_grid_to_world = GridToWorld::scale_um(2.0, 2.0, 2.0);
    ...
    let camera = CameraState::new(
        ...
        PresentationViewport::new_unchecked(10.0, 10.0),
    );
...
fn uint8_camera_volume() -> DenseVolumeU8 {
    DenseVolumeU8::new(
        ...
        GridToWorld::scale_um(1.0, 1.0, 1.0),
        vec![
            3, 25, 80, //
            4, 5, 6, //
            2, 225, 10, //
